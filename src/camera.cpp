...
Vec3 Camera::world2camera(const Vec3& p_w, const SE3& T_cw) const {
    // 此处 pose 是相机如何从原点移动到当前位置, 也即 T_wc.
    return T_cw * p_w;
}

Vec3 Camera::camera2world(const Vec3& p_c, const SE3& T_cw) const {
...
Vec2 Camera::world2pixel(const Vec3& p_w, const SE3& T_cw) const {
    return camera2pixel(world2camera(p_w, T_cw));
}

Vec3 Camera::pixel2world(const Vec2& p_uv, const SE3& T_cw, double depth) const {
